...
actor:
			...
			| recv = receiver 
				{
					...
					if(MaxArgSize < $recv.argSize)
						MaxArgSize = $recv.argSize;
				}
			| NL
			)*
		...
		{ 
			...
			try {
            	addActor($act_id.text, arg, MaxArgSize,Integer.parseInt($num.text));
	        }
	        catch(ItemAlreadyExistsException e) {
	        	...
	        	while(true){
	        		try{
	        			addActor(s + Integer.toString(cnt), arg, MaxArgSize,Integer.parseInt($num.text));
	        		}
	        		catch(ItemAlreadyExistsException exp){
	        			cnt++;
	        			continue;
...
receiver returns [String signature, int argSize]:
		{ beginScope(); }
		{ args.clear(); $argSize = 0;}
		'receiver' recv_id = ID '(' (var_type = type var_id = ID { $argSize+= $var_type.return_type.size(); args.add($var_type.return_type.toString()); addLocalVar( $var_id.text,$var_type.return_type,$var_id.getLine() ); } 
			(','var_type = type var_id = ID { $argSize+= $var_type.return_type.size(); args.add($var_type.return_type.toString()); addLocalVar( $var_id.text,$var_type.return_type,$var_id.getLine() ); }  )*)? ')' NL
		{ 
			try {
